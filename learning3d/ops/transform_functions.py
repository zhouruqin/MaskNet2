...
class SampleNetTransform:
	...
	def generate_transform(self):
		...
		angle = 2*self.max_rotation*torch.rand(1).numpy()[0] - self.max_rotation
	...
	def apply_transform(self, p0):
		p1 = torch.matmul(self.igt[0:3, 0:3], p0.permute(1,0)).permute(1,0) + self.igt[0:3, 3]
		return p1
...
class PCRNetTransform:
	...
	def __call__(self, point_cloud, index):
		...
		source = self.quaternion_rotate(point_cloud, gt)
		return source
