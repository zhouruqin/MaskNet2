...
class PRNet(nn.Module):
    ...
    def forward(self, *input):
        ...
        if len(input) == 2:
            tgt, src = input[0], input[1]
        elif len(input) == 3:
            tgt, src, rotation_ab, translation_ab = input[0], input[1], input[2][:, :3, :3], input[2][:, :3, 3].view(-1, 3)
            calculate_loss = True
        elif len(input) == 4:
            tgt, src, rotation_ab, translation_ab = input[0], input[1], input[2], input[3]
            calculate_loss = True
