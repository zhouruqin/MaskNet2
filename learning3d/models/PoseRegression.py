...
class pose_regression_CMA(nn.Module):
    def __init__(self, bottleneck_size=1024, input_shape="bcn"):
        ...
        self.fc1_ = BasicConv1D(1024, 512)#nn.Sequential(Mish(),nn.Linear(1024, 512) )#
       
        self.fc2 = self_attention_fc(512, 256)#
        self.fc2_ = BasicConv1D(512, 256)#nn.Sequential( Mish(), nn.Linear(512, 256))#
    
        self.fc3 = self_attention_fc(256, 128)#
        self.fc3_ = BasicConv1D(256, 128)#nn.Sequential( Mish(), nn.Linear(256, 128))# 
    ...
    def forward(self, x, y): #B, 1024
        ...
        delt_xy_0 = self.fc1_(torch.cat([x0, y0], dim=1))
       
        x1, y1= self.fc2( x0, y0)    #256
        delt_xy_1 = self.fc2_(torch.cat([x1, y1], dim=1)+delt_xy_0 )
      
        x2, y2=self.fc3(x1, y1)    #128
        delt_xy_2 = self.fc3_(torch.cat([x2, y2], dim=1)+delt_xy_1)
        ...
        #x4 = self.fc2_(x3)
        #x5 = self.fc3_(x4)
        x6 = self.fc6(delt_xy_2.squeeze(2) )#x5.squeeze(2)
